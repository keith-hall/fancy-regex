...
fn main() {
    ...
    // Test reverse search on "Carefuly" - should find "fuly" when searching backwards
    let text = "Carefuly";
    let input = Input::new(text)
        .span(0..text.len())
        .anchored(Anchored::Yes);
    
    println!("Testing reverse search on '{}' (len={})", text, text.len());
    match dfa.try_search_rev(&mut cache, &input) {
        Ok(Some(m)) => println!("Found match at offset: {}", m.offset()),
