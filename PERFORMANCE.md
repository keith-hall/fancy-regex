...
fancy-regex now has a **Seek** pre-filter optimisation (enabled by
...
For lookahead-only hard parts the approximation drops the lookahead, so
the Seek pre-filter does not directly replicate Oniguruma's "find `c`
first" step in that specific case. The Seek optimisation can be turned
off via `RegexBuilder::seek(false)` or
`RegexOptionsBuilder::seek(false)`.
...
* With fancy features, Oniguruma can still be faster in some cases. Its
  ...
  (for example, lookahead-only patterns where the Seek approximation
  drops the lookahead). The Seek pre-filter can be turned off via
  `RegexBuilder::seek(false)` if needed.
